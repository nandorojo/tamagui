...
Before we get lost in the weeds, lets get into why we even want templates. To do that we need to first understand that we often share the same shape of themes across different sub-themes, and so the idea of a generic template is useful.
